#!/usr/bin/env python3
"""
KrillClaw Bridge — connects a BLE/Serial KrillClaw device to Claude API.

The bridge receives RPC messages from KrillClaw running on embedded hardware,
...
Usage:
  # BLE mode (connect to KrillClaw ring/device):
  python bridge.py --ble

  # Serial mode (connect to KrillClaw dev board):
...
ROBOT_LOG_PATH = os.path.expanduser("~/.krillclaw/robot_commands.log")
...
def handle_web_search(data):
    ...
    try:
        ...
        req = urllib.request.Request(url, headers={
            "User-Agent": "KrillClaw/0.1.0 (AI Agent Runtime)",
        })
...
def handle_session_save(data):
    ...
    session_dir = os.path.expanduser("~/.krillclaw/sessions")
...
def handle_session_load(data):
    ...
    path = os.path.join(os.path.expanduser("~/.krillclaw/sessions"),
                        f"{session_id}.json")
    try:
...
def handle_ota_check(data):
    ...
    repo = data.get("repo", "krillclaw/KrillClaw")
    current_version = data.get("current_version", "0.1.0")

    try:
        url = f"https://api.github.com/repos/{repo}/releases/latest"
        req = urllib.request.Request(url, headers={
            "User-Agent": "KrillClaw/0.1.0",
...
def handle_ota_apply(data):
    ...
    try:
        ...
        return {
            ...
            "backup": backup,
            "message": "Update applied. Restart KrillClaw to use the new version.",
        }
    except Exception as e:
...
async def socket_server(bridge: KrillClawBridge, path: str):
    ...
    print(f"[bridge] Waiting for KrillClaw device...")
    async with server:
        await server.serve_forever()


async def serial_bridge(bridge: KrillClawBridge, port: str, baud: int = 115200):
...
async def ble_bridge(bridge: KrillClawBridge):
    """BLE bridge using bleak (scans for KrillClaw device)."""
    try:
    ...
    print("[bridge] Scanning for KrillClaw BLE device...")
    ...
    if not device:
        print("[bridge] No KrillClaw device found")
        return
...
logger = logging.getLogger("krillclaw.telegram")


class TelegramTransport:
    """Telegram Bot API transport using HTTP long-polling.

    Polls for incoming text messages, invokes the KrillClaw agent (via
...
def main():
    ...
    if args.transport == "telegram":
        ...
        asyncio.run(socket_server(bridge, "/tmp/krillclaw.sock"))
